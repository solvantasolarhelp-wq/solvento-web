...
export default function AdminLogin() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const { user, error: err } = await adminLogin(email, password);
    setLoading(false);
    if (!user) { setError(err || "Login failed."); return; }
    router.push("/admin");
  }
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="glass-panel-dark rounded-3xl p-8 border border-white/10 shadow-2xl">
          ...
          <div className="mt-6 pt-5 border-t border-white/5 text-center space-y-3">
            <a href="/" className="text-xs text-white/40 hover:text-white/70 transition-colors block">← Back to Website</a>
